...
async function handler(req: AuthRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Users (non-admin) with current work status for the employees page.
    const users = await User.find({ role: { $ne: 'admin' } }, 'name email dni role registered groups expectedWorkHours workDays avatar')
      .sort({ name: 1 })
      .lean() as any[];
    ...
    const workingUserIds = new Set(
      latestSessions.filter((s: any) => s.latest.type === 'check_in').map((s: any) => s._id)
    );
    ...
    const weekSessions = await WorkSession.find({ timestamp: { $gte: monday, $lte: weekEnd } }).sort({ timestamp: 1 }).lean();
    ...
    for (const user of users) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const userSessions = sessionsByUserDay.get(`${user._id}:${dateKey(day)}`) ?? [];
        const expectedHours = user.expectedWorkHours ?? settings.defaultExpectedHours;
        ...
        else if (!(totalHours >= expectedHours - settings.toleranceHours && totalHours <= expectedHours + settings.toleranceHours)) {
