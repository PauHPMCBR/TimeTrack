...
const AdminExportRequestSchema = ExportRequestSchema.extend({
    userIds: z
        .array(z.string().min(1))
        .min(1, 'At least one user id is required'),
});

type AdminExportRequest = z.infer<typeof AdminExportRequestSchema>;

export default withApi(
    {
        method: 'POST',
        guard: 'admin',
        body: AdminExportRequestSchema,
        audit: {
            action: 'export_work_sessions',
            targetType: 'work_session_export',
            metadata: (_req, ctx) => {
                const body = ctx.body as AdminExportRequest;
                return {
