...
import { User, WorkSession, ElectiveVacation, Group, YearlyVacationDays, WorkSessionReason } from '@/models';
import { responseErrorPost, responseErrorMethodNotAllowed } from '@/lib/response-error-generator';
...
async function handler(req: AuthRequest, res: NextApiResponse) {
  ...
  try {
    ...
    res.status(200).json({ success: true, indexes: results });
  } catch (error) {
