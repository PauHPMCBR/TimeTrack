...
async function handler(req: AuthRequest, res: NextApiResponse) {
  ...
  const validationMiddleware = validateRequestBody(CopyYearlyVacationRequestSchema);
  ...
  try {
    ...
    // Shift obligatory days to the same month/day of the target year.
    const obligatoryDays = (source.obligatoryDays ?? []).map((date: Date) => {
