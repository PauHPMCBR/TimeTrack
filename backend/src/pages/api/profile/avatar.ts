...
import { responseErrorGet, responseErrorIncorrectParameter, responseErrorMethodNotAllowed, responseErrorPost } from '@/lib/response-error-generator';
...
async function handler(req: AuthRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (buffer.length === 0 || buffer.length > AVATAR_MAX_BYTES) {
      return responseErrorIncorrectParameter(res, 'avatar', ['AvatarTooLarge']);
    }
    ...
    try {
      ...
      return responseErrorIncorrectParameter(res, 'avatar', ['InvalidAvatarFormat']);
    }
