...
export default function GroupModal({ open, group, onClose, onSaved }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.error) {
      setError(t(`error.${res.error}`) === `error.${res.error}` ? res.error : t(`error.${res.error}`));
      return;
    }
  ...
  return (
    <Modal open={open} title={group ? t("admin.groups.editTitle") : t("admin.groups.createTitle")} onClose={requestClose}>
      {error && (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold ${isSelected ? "bg-indigo-600 text-white" : "bg-zinc-200 text-zinc-600 dark:bg-zinc-700 dark:text-zinc-300"}`}>
                          {initial}
                        </div>
                        <div>
                          <p className={`text-sm font-medium ${isSelected ? "text-indigo-700 dark:text-indigo-300" : "text-zinc-700 dark:text-zinc-300"}`}>{displayName}</p>
                          <p className="text-xs text-zinc-500">{user.email}</p>
                        </div>
                      </div>
                      <div className={`flex h-5 w-5 items-center justify-center rounded-full border ${isSelected ? "border-indigo-600 bg-indigo-600" : "border-zinc-300 bg-white dark:border-zinc-600 dark:bg-zinc-800"}`}>
