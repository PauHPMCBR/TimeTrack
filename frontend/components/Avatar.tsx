...
export default function Avatar({
  ...
  if (!src) {
    return (
      <div className={`grid shrink-0 place-items-center overflow-hidden ${fallbackClassName}`}>
        <span className="font-bold">{fallback}</span>
      </div>
    );
  }

  return <img src={src} alt={alt} className={`shrink-0 overflow-hidden ${className}`} />;
}