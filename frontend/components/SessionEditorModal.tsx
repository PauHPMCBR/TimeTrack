...
function nextExpectedType(sessions: EditableSession[]): "check_in" | "check_out" {
...
export default function SessionEditorModal({ row, onClose, onSaved }: Props) {
  ...
  const [sessions, setSessions] = useState<EditableSession[]>(
    row.sessions.map((s) => ({ _id: s._id, type: s.type, timestamp: toDatetimeLocal(s.timestamp) }))
  );
  ...
  const handleChangeTime = (session: EditableSession, value: string) => {
    setSessions((prev) => prev.map((s) => (s._id === session._id ? { ...s, timestamp: value } : s)));
    markDirty();
  };
  ...
  const handleSave = async () => {
    ...
    if (res.error) {
      if (res.error === 'IncorrectParameter' && Array.isArray(res.details?.reasons) && res.details.reasons.length > 0) {
        const key = `error.IncorrectParameter.reason.${res.details.reasons[0]}`;
        const text = t(key);
        setError(text !== key ? text : t("error.IncorrectParameter.message"));
      } else {
        setError(t(`error.${res.error}`) === `error.${res.error}` ? t("error.PutError") : t(`error.${res.error}`));
      }
      return;
    }
      ...
      >
        <div className="mb-4 flex items-start justify-between">
          <div>
            ...
            <p className="mt-0.5 text-sm text-zinc-500">{row.userName} · {dateLabel}</p>
          </div>
          <button
            onClick={requestClose}
        ...
        <div className="space-y-2">
          ...
          {sessions.map((session, idx) => {
            const removable = isCoherent(sessions.filter((_, i) => i !== idx));
            return (
              <div
                key={session._id}
              ...
              >
                ...
                >
                  {session.type === "check_in" ? <LogIn size={14} /> : <LogOut size={14} />}
                </span>
                <input
                  type="time"
                  value={session.timestamp.split("T")[1] ?? ""}
                  disabled={saving}
                  onChange={(e) => handleChangeTime(session, `${row.date}T${e.target.value}`)}
                ...
                <button
                  ...
                  title={removable ? t("admin.sessionEditor.remove") : t("admin.sessionEditor.removeLocked")}
                >
                  <Trash2 size={16} />
                </button>
              </div>
        ...
        <div className="mt-4 flex justify-end gap-2">
          <Button onClick={requestClose} disabled={saving} variant="secondary">
            {t("common.cancel")}
          </Button>
