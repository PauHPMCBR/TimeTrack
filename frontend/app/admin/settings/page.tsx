      ...
      try {
        ...
        if (res.error) {
          ...
          setFormData({
            ...
            toleranceHours: s.toleranceHours ?? s.benevolenceHours ?? 1,
          ...
          <div className="space-y-6">
            <div>
              ...
              <HoursMinutesInput
                ...
                minHours={0.5}
                onChange={(v) => updateForm({ defaultExpectedHours: v })}
              />
            ...
            <div>
              ...
              <HoursMinutesInput
                ...
                minHours={0}
                onChange={(v) => updateForm({ toleranceHours: v })}
              />
              <p className="mt-1.5 text-xs text-zinc-500">{t("admin.settings.toleranceHelp")}</p>
            </div>
            ...
            <div>
              ...
              <HoursMinutesInput
                ...
                minHours={0}
                onChange={(v) => updateForm({ endOfDayHour: Math.min(24, v) })}
              />
              <p className="mt-1.5 text-xs text-zinc-500">{t("admin.settings.endOfDayHelp")}</p>
            </div>

            <div>
              <Label className="mb-2">{t("admin.settings.nonWorkingDaysLabel")}</Label>
              <WeekDaysSelector
                selected={formData.nonWorkingDays}
                onToggle={toggleDay}
                locale={localeTag(lang)}
              />
              <p className="mt-1.5 text-xs text-zinc-500">{t("admin.settings.nonWorkingDaysHelp")}</p>
            </div>
          </div>

          <Button type="submit" disabled={saving} variant="primary" className="mt-8 w-full">
