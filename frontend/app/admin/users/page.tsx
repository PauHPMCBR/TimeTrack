...
import { UserPlus, Download, Pencil, Timer, Users, TriangleAlert } from "lucide-react";
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 flex items-center gap-4 text-sm text-zinc-500">
            <span className="flex items-center gap-1"><Users size={14} />{loading ? "-" : total} {t("admin.menu.users.total")}</span>
            <span className="flex items-center gap-1"><Timer size={14} />{loading ? "-" : workingNow} {t("admin.menu.users.working")}</span>
          </p>
        </div>
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <ul className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {users.length === 0 && (
              <div className="p-8 text-center text-sm text-zinc-500">{t("admin.users.notFound")}</div>
            )}
            {users.map((user) => (
              <li key={user._id} className="flex items-center justify-between p-4 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                    ...
                    <div className="font-medium text-zinc-900 dark:text-white">
                      ...
                      {user.workDays && user.workDays.length > 0 && (
                        <span className="ml-2 inline-flex items-center gap-1 rounded-md bg-amber-100 px-1.5 py-0.5 text-xs font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">
                          ...
                          {t("admin.users.customNonWorkDays")}
                        </span>
                      )}
                      {user.role === 'admin' && (
