  ...
  return (
    <div className="space-y-6">
        ...
        {loading ? (
          ...
          groups.map((group) => (
            <Card key={group._id} className="flex flex-col p-5">
              <div>
                <h3 className="truncate font-semibold text-zinc-900 dark:text-white">{group.name}</h3>
                <p className="mb-3 text-sm text-zinc-500">{group.description || t("admin.groups.noDesc")}</p>

                <div className="mb-4">
                  <p className="mb-1 text-xs font-medium text-zinc-500">{t("admin.groups.members")}</p>
                  {group.members.length === 0 ? (
                    <p className="text-xs text-zinc-400">{t("admin.groups.noMembers")}</p>
                  ) : (
                    <div className="flex flex-wrap gap-1.5">
                      {group.members.map((m) => (
                        ...
                        >
                          {userMap[m] || t("admin.users.noName")}
                        </span>
                      ))}
                    </div>
                  )}
      ...
      {isDeleteModalOpen &&
        ...
        createPortal(
          ...
          <div className="w-full max-w-sm rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <h3 className="text-lg font-semibold text-zinc-900 dark:text-white">{t("admin.groups.deleteConfirmTitle")}</h3>
            <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">{t("admin.groups.deleteConfirmDesc")}</p>
            {errorMsg && (
              <div className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-600 dark:bg-red-900/20 dark:text-red-400">{errorMsg}</div>
            )}
            <div className="mt-6 flex gap-3">
              <Button onClick={closeDeleteModal} disabled={isDeleting} variant="secondary" className="flex-1">
                {t("common.cancel")}
              </Button>
      ...
      <GroupModal
        ...
        group={editingGroup}
        onClose={() => { setCreating(false); setEditingGroup(null); }}
        onSaved={fetchData}
      />
    </div>
  );
