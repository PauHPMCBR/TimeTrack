...
import { ChevronRight, ChevronLeft, ShieldCheck, User, Zap } from "lucide-react";
...
function AdminEventsInner() {
  ...
  const [period, setPeriod] = useState<Period>(() => {
    ...
    return p === "day" || p === "week" || p === "month" || p === "year" ? p : "week";
  });
  ...
  const periodLabel = () => {
    ...
    const start = cursor.toLocaleDateString(locale, { day: "numeric", month: "short", year: "numeric" });
  ...
  const rowClass = (row: AdminWorkSessionRow) => {
    if (row.status === "vacation") return "bg-blue-50/70 dark:bg-blue-950/30";
    if (row.status === "ok") return "bg-green-50/70 dark:bg-green-950/30";
    if (row.status === "nonWorkingDay") return "bg-zinc-50/70 dark:bg-zinc-800/40";
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      <div className="mx-auto max-w-6xl px-4 py-6">
        ...
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">{t("admin.events.title")}</h1>
        ...
        {loading ? (
          ...
          <Card className="p-10 text-center text-sm text-zinc-500">{t("admin.events.noData")}</Card>
        ) : (
              ...
              <table className="w-full table-fixed text-left text-sm">
                <thead className="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900/50">
                  <tr className="text-xs uppercase tracking-wider text-zinc-500">
                    <th className="w-[110px] whitespace-nowrap px-3 py-3 font-semibold">{t("admin.events.table.date")}</th>
                    <th className="w-[150px] px-3 py-3 font-semibold">{t("admin.events.table.employee")}</th>
                    <th className="px-3 py-3 font-semibold">{t("admin.events.table.sessions")}</th>
                    <th className="w-[70px] whitespace-nowrap px-3 py-3 text-right font-semibold">{t("admin.events.table.hours")}</th>
                    <th className="w-[96px] whitespace-nowrap px-3 py-3 text-right font-semibold">{t("admin.events.table.expected")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                  {rows.map((row) => {
                    const dateLabel = new Date(`${row.date}T00:00:00`).toLocaleDateString(locale, {
                      weekday: "short",
                      day: "numeric",
                      month: "short",
                    });
                    return (
                      ...
                      >
                        ...
                        <td className="truncate px-3 py-3 font-medium text-zinc-900 dark:text-white">{row.userName}</td>
                          ...
                          <div className="flex flex-wrap items-center gap-1.5">
                            {row.sessions.length === 0 ? (
                              ...
                              row.sessions.map((s, i) => {
                                ...
                                const SourceIcon = source === "admin" ? ShieldCheck : source === "automatic" ? Zap : User;
                                return (
                        ...
                        <td className="whitespace-nowrap px-3 py-3 text-right font-medium text-zinc-900 dark:text-white">
                          {row.totalHours > 0 ? formatHM(row.totalHours * 3_600_000, t) : "—"}
                        </td>
